...
with torch.no_grad():
    for batch_idx, (images_test, labels_test) in enumerate(test_loader):
        ...
        # === STEP B: Decide if we do local analysis ===
        #    1) If it's from '001.good', only do local analysis if we've done < MAX_GOOD_ANALYSIS
        #    2) If it's from any other class, always do local analysis.
        class_folder = test_dataset.classes[labels_test.item()]  # e.g. "001.good" or "002.defect", etc.
        do_local_analysis = False
        if class_folder == '001.good':
            if analysis_count['001.good'] < MAX_GOOD_ANALYSIS:
                do_local_analysis = True
                analysis_count['001.good'] += 1
        else:
            # For defective classes, always do local analysis
            do_local_analysis = True

        # === STEP C: Run local analysis ONLY if do_local_analysis == True ===
