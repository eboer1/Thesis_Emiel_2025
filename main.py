...
# Get current date and time to use as a unique identifier
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

model_dir = './saved_models/' + base_architecture + '/' + experiment_run + '_' + current_time + '/'
makedir(model_dir)
...
log, logclose = create_logger(log_filename=os.path.join(model_dir, 'train_log.txt'))
...
from settings import train_batch_size, test_batch_size, train_push_batch_size, num_classes
...
log(f"Configured num_classes: {num_classes}")

log(f"Prototype Shape Being Passed to Model: {prototype_shape}")
...
'''
# load old model for incremental learning
old_model_path = '/home2/s4991125/Deformable-ProtoPNet-Incremental/saved_models/resnet34/001_default_20250114_160536/15push0.7500.pth' # load here old model path
old_model = torch.load(old_model_path)  # loading the entire old model
old_model.eval()
for param in old_model.parameters():
    param.requires_grad = False #freeze parameters

old_num_classes = 3  # this should match the old model's number of classes

print(f"In main.py: old_model is of type {type(old_model)} and old_num_classes={old_num_classes}")
'''

# define old model and classes as none if incremental learning not used
old_model = None
old_num_classes = None


# Calculate and log the total trainable parameters
from functools import reduce

# Calculate total parameters in the entire model
total_params = sum(p.numel() for p in ppnet.parameters() if p.requires_grad)

# Calculate prototype layer parameters separately
prototype_vectors = ppnet.prototype_vectors  # Assuming `prototype_vectors` exists in the model
num_prototypes = prototype_vectors.shape[0]  # K
prototype_shape = prototype_vectors.shape[1:]  # (C, H, W)
prototype_layer_params = reduce(lambda x, y: x * y, prototype_shape) * num_prototypes

# Log the results
log(f"Total Trainable Parameters: {total_params}")
log(f"Prototype Layer Parameters (K * C * H * W): {prototype_layer_params}")
log(f"Parameters in Rest of the Model: {total_params - prototype_layer_params}")
...
# Add confusion matrix directory inside the model_dir
confusion_matrix_dir = os.path.join(model_dir, 'confusion_matrices')
...
for epoch in range(num_train_epochs):
    ...
    if epoch < num_warm_epochs:
        ...
        _ = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=warm_optimizer,
                    ...
                    use_ortho_loss=False, old_model=old_model, old_num_classes=old_num_classes)
        ...
        _ = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=joint_optimizer,
                    class_specific=class_specific, coefs=coefs, log=log, subtractive_margin=subtractive_margin,
                    use_ortho_loss=True, old_model=old_model, old_num_classes=old_num_classes)
